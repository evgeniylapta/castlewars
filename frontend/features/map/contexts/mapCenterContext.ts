...
function useCenterPointHandle(
  initialPoint: Point,
  goToMyCastlePointCallback?: () => void,
  selectedPoint?: Point
) {
  ...
  return {
    ...
    goToSelectedCastle: useCallback(
      () => selectedPoint && setCurrentCenterPoint(selectedPoint),
      [selectedPoint]
    ),
...
function useMapControlsHandle(
  ...
  setCurrentCenterPoint: (point: Point) => void
) {
  return {
    goLeft: useCallback(
      () => setCurrentCenterPoint({ ...currentCenterPoint, x: currentCenterPoint.x - 1 }),
      [currentCenterPoint]
    ),
    goRight: useCallback(
      () => setCurrentCenterPoint({ ...currentCenterPoint, x: currentCenterPoint.x + 1 }),
    ...
    goTop: useCallback(
      () => setCurrentCenterPoint({ ...currentCenterPoint, y: currentCenterPoint.y + 1 }),
      [currentCenterPoint]
    ),
    goBottom: useCallback(
      () => setCurrentCenterPoint({ ...currentCenterPoint, y: currentCenterPoint.y - 1 }),
      [currentCenterPoint]
    )
  }
...
const useContext = () => {
  ...
  const useCenterPointHandleReturn = useCenterPointHandle(
    myCastlePoint,
  ...
  return {
    ...
    ...useMapControlsHandle(
      useCenterPointHandleReturn.currentCenterPoint,
      useCenterPointHandleReturn.setCurrentCenterPoint
    )
  }
}
