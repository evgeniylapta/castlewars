...
export function realTimeHandleStart() {
  ...
  asyncTimerStart(async () => {
    ...
    await executeConsideringTickInterval(counter, ATTACKS_PROCESSING_TICK, attacksProcessingTick)
    await executeConsideringTickInterval(counter, HANDLE_UNIT_ORDERS_TICK, handleUnitOrders)
    await executeConsideringTickInterval(counter, BOTS_ACTIONS_EXECUTE_TICK, botsActionsExecuteTick)
    await executeConsideringTickInterval(counter, BOTS_ACTIONS_CREATING_TICK, botsActionsCreatingTick)
