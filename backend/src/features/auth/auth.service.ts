...
export async function signUp ({ email, password, name, tribeTypeId }: PostSignUpDto) {
...
export async function refreshToken ({ refreshToken }: PostRefreshTokenDto) {
  try {
    const foundRefreshToken = await verifyToken(refreshToken, 'REFRESH');
